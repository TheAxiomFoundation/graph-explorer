...
|---|---|---|
| Wider aisles respond to the stage aspect; leaf pairs reduce tall question stacks. | Fixed-spacing Dagre layout. | Add a layout extension receiving viewport dimensions and node measurements. Port the layout with its geometry checks; pairing eligibility and question meaning remain Axiom-owned. |
| Titled cards use a common width and content-driven height, including answers and action rows. | Fixed 248 × 126 cards; custom content is available in the inspector. | Add custom node renderers, per-node sizing, and handle/edge-routing contracts before replacing Axiom’s canvas. |
